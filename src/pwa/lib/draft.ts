...
// exist? Set both when a save actually lands, and when loadDraft() finds a
// fresh one — a crash between "draft loaded" and "user clicked Resume" must
// still tell the truth about recoverability.
let savedDraftFlag = false;
export function hasSavedDraft(): boolean {
  return savedDraftFlag;
}
